...
def save_image_grid(img, fname, drange, grid_size, quality=80):
    lo, hi = drange
    ...
    assert C in [1, 3]
    if C == 1:
        PIL.Image.fromarray(img[:, :, 0], 'L').save(fname, quality=quality)
    if C == 3:
        PIL.Image.fromarray(img, 'RGB').save(fname, quality=quality)
